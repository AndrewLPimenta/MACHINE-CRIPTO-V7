...
toggleButton.addEventListener("click", () => {
  if (cryptoList1.style.display === "none") {
    // Exibe a primeira lista e oculta a segunda
    ...
    // Exibe a segunda lista e oculta a primeira
    cryptoList1.style.display = "none";
    cryptoList2.style.display = "flex"; // Usa "flex" para manter a estrutura de lista
...
function toggleSeta(id) {
  ...
  var seta = elemento.querySelector(".seta"); // Seleciona a seta
  var estaMostrando = elemento.classList.contains("mostrar");

  // Altera a seta dependendo do estado do acordeão
  if (!estaMostrando) {
    seta.textContent = "expand_less"; // Seta para cima
  } else {
    seta.textContent = "expand_more"; // Seta para baixo
  }
}
